...
if [[ $(uname -m) == "arm64" ]]; then
  cargo build --release
  cp target/release/dermo "$DIST/dermo-macos-aarch64"
  echo "    -> $DIST/dermo-macos-aarch64"
else
  echo "    Cross-compiling (host is $(uname -m))..."
  rustup target add aarch64-apple-darwin 2>/dev/null || true
  cargo build --release --target aarch64-apple-darwin
  cp target/aarch64-apple-darwin/release/dermo "$DIST/dermo-macos-aarch64"
  echo "    -> $DIST/dermo-macos-aarch64"
fi
...
if command -v cargo-zigbuild &>/dev/null; then
  ...
  cp target/x86_64-pc-windows-gnu/release/dermo.exe "$DIST/dermo-windows-x86_64.exe"
  echo "    -> $DIST/dermo-windows-x86_64.exe"
